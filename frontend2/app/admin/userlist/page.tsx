        ...
        <Table>
          ...
          <TableBody>
            {users.map((user: any) => {
                ...
                const canDelete = !targetIsSuperAdmin && (isSuperAdmin || !targetIsAdmin);

                return (
                  <TableRow key={user._id}>
                    <TableCell className="font-medium">
                      <div className="flex flex-col">
                        <span className="text-base">{user.name} {user.lastName}</span>
                    ...
                    <TableCell>
                      <Badge 
                        className={
                          ...
                          user.role === "admin" ? "bg-blue-600 hover:bg-blue-700" : "bg-slate-500"
                        }
                      >
                        {user.role === "superadmin" ? "Super Admin" : user.role === "admin" ? "Admin" : "Korisnik"}
                      </Badge>
                    </TableCell>
                    <TableCell>
                        ...
                        {isSuperAdmin && !targetIsSuperAdmin && (
                          <form action={async () => {
                            ...
                            <Button 
                              size="icon" 
                              variant="ghost" 
                              title={user.role === "admin" ? "Ukloni admin prava" : "Dodijeli admin prava"}
                            >
                              {user.role === "admin" ? (
